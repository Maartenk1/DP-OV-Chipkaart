...
FROM (((inschrijvingen
    LEFT JOIN uitvoeringen ON inschrijvingen.cursus = uitvoeringen.cursus)
    LEFT JOIN medewerkers AS docent ON uitvoeringen.docent = docent.mnr)
         LEFT JOIN medewerkers AS cursist ON inschrijvingen.cursist = cursist.mnr)
...
SELECT afdelingen.naam AS afdeling, medewerkers.naam AS naam FROM afdelingen
                                                                      INNER JOIN medewerkers
                                                                                 ON (hoofd = mnr);
...
SELECT naam, toelage FROM medewerkers, schalen
WHERE (maandsal >= ondergrens AND maandsal <= bovengrens);
