...
SELECT AGE(max(gbdatum),min(gbdatum)), AGE(to_timestamp(avg(extract(epoch from gbdatum)))::date) FROM medewerkers;
...
SELECT COUNT(mnr) AS aantal_medewerkers, (sum(comm) / COUNT(mnr)) as commissie_medewerkers, AVG(comm) AS commissie_verkopers FROM medewerkers;
